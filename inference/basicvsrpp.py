...
"""BasicVSR++ NTIRE compressed-video preprocessor for the full-frame pipeline.

Inference-oriented adaptation of OpenMMLab MMagic's BasicVSR++ implementation.
The NTIRE decompression checkpoint restores at the source resolution. Spatial
processing may be tiled for VRAM safety; downstream Real-ESRGAN remains full-frame.
"""
...
class ResidualBlockNoBN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return x + self.conv2(self.relu(self.conv1(x))) * self.residual_scale
...
class SPyNet(nn.Module):
    ...
    def compute_flow(self, ref: torch.Tensor, supp: torch.Tensor) -> torch.Tensor:
        ...
        for level, (ref_level, supp_level) in enumerate(zip(ref_pyramid, supp_pyramid)):
            flow_up = flow if level == 0 else F.interpolate(
                flow, scale_factor=2, mode="bilinear", align_corners=True
            ) * 2.0
            warped = flow_warp(supp_level, flow_up.permute(0, 2, 3, 1), padding_mode="border")
    ...
    def forward(self, ref: torch.Tensor, supp: torch.Tensor) -> torch.Tensor:
        ...
        flow = F.interpolate(
            self.compute_flow(ref_up, supp_up), size=(h, w), mode="bilinear", align_corners=False
        )
...
class SecondOrderDeformableAlignment(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, extra_feat: torch.Tensor, flow_1: torch.Tensor, flow_2: torch.Tensor
    ) -> torch.Tensor:
...
class BasicVSRPlusPlusNet(nn.Module):
    """Checkpoint-compatible BasicVSR++ topology for same-resolution restoration."""

    def __init__(self, mid_channels: int = 128, num_blocks: int = 25, max_residue_magnitude: int = 10):
        super().__init__()
        ...
        for index, name in enumerate(("backward_1", "forward_1", "backward_2", "forward_2")):
    ...
    def compute_flow(self, lqs: torch.Tensor) -> tuple[Optional[torch.Tensor], torch.Tensor]:
        ...
        backward = self.spynet(lqs_1, lqs_2).view(n, t - 1, 2, h, w)
        forward = None if self.is_mirror_extended else self.spynet(lqs_2, lqs_1).view(n, t - 1, 2, h, w)
        return forward, backward

    def propagate(
        self, feats: dict[str, list[torch.Tensor]], flows: torch.Tensor, module_name: str
    ...
    def upsample(self, lqs: torch.Tensor, feats: dict[str, list[torch.Tensor]]) -> torch.Tensor:
        ...
        count = len(feats["spatial"])
        mapping_idx = list(range(count)) + list(range(count))[::-1]
        for i in range(lqs.size(1)):
            hr = [feats[key].pop(0) for key in feats if key != "spatial"]
            hr.insert(0, feats["spatial"][mapping_idx[i]])
            out = self.reconstruction(torch.cat(hr, dim=1))
            out = self.lrelu(self.upsample1(out))
            out = self.lrelu(self.upsample2(out))
            out = self.lrelu(self.conv_hr(out))
            out = self.conv_last(out) + lqs[:, i]
            outputs.append(out)
    ...
    def forward(self, lqs: torch.Tensor) -> torch.Tensor:
        ...
        if t < 2:
            return lqs
        if h % 4 or w % 4:
            raise ValueError(f"BasicVSR++ input must be divisible by 4, got {w}x{h}")
        downsampled = F.interpolate(
            lqs.reshape(-1, c, h, w), scale_factor=0.25, mode="bicubic", align_corners=False
        ).view(n, t, c, h // 4, w // 4)
        if downsampled.shape[-2] < 64 or downsampled.shape[-1] < 64:
            raise ValueError("BasicVSR++ decompression tiles must be at least 256x256")
        self.check_if_mirror_extended(lqs)
        feat_tensor = self.feat_extract(lqs.reshape(-1, c, h, w))
        fh, fw = feat_tensor.shape[-2:]
        feat_tensor = feat_tensor.view(n, t, -1, fh, fw)
        feats: dict[str, list[torch.Tensor]] = {
            "spatial": [feat_tensor[:, i] for i in range(t)]
        }
        flows_forward, flows_backward = self.compute_flow(downsampled)
        for iteration in (1, 2):
            for direction in ("backward", "forward"):
                ...
                feats = self.propagate(feats, flows, name)
...
def download_checkpoint(target_dir: Path) -> Path:
    target_dir.mkdir(parents=True, exist_ok=True)
    target = target_dir / BASICVSRPP_TRACK1_URL.rsplit("/", 1)[-1]
    if target.is_file() and target.stat().st_size > 0:
        print(f"[basicvsrpp] using cached checkpoint: {target}", flush=True)
        return target
    temporary = target.with_suffix(target.suffix + ".part")
    print(f"[basicvsrpp] downloading {BASICVSRPP_TRACK1_URL}", flush=True)
    try:
        urllib.request.urlretrieve(BASICVSRPP_TRACK1_URL, temporary)
...
def normalize_state(raw_state: dict[str, torch.Tensor], model: nn.Module) -> dict[str, torch.Tensor]:
    ...
    for original_key, value in raw_state.items():
        ...
        if key in model_keys:
            ...
            raise KeyError(f"Unexpected BasicVSR++ generator key: {original_key}")
    hard_missing = sorted(model_keys - set(normalized) - {"spynet.mean", "spynet.std"})
    if hard_missing:
        raise KeyError(f"BasicVSR++ checkpoint is missing {len(hard_missing)} model keys: {hard_missing[:8]}")
    return normalized


def load_checkpoint(model: nn.Module, path: Path) -> None:
    state = normalize_state(_extract_state_dict(_torch_load_cpu(path)), model)
    result = model.load_state_dict(state, strict=False)
    hard_missing = sorted(set(result.missing_keys) - {"spynet.mean", "spynet.std"})
    ...
    print(f"[basicvsrpp] checkpoint loaded: keys={len(state)}, path={path}", flush=True)
...
def float_rgb_to_dtype(frame: np.ndarray, dtype: np.dtype) -> np.ndarray:
    ...
    if dtype.kind == "u" and dtype.itemsize == 2:
        return np.ascontiguousarray(np.rint(clipped * 65535.0).astype(dtype))
    if dtype == np.dtype(np.float32):
        return np.ascontiguousarray(clipped, dtype=np.float32)
    raise TypeError(f"Unsupported BasicVSR++ output dtype: {dtype}")
...
class BasicVSRPPPreprocessor:
    def __init__(self, config: BasicVSRPPConfig, checkpoint_dir: Path):
        self.config = config
        ...
        if config.tile_size < 256 or config.tile_size % 4:
            raise ValueError("BasicVSR++ tile size must be >=256 and divisible by 4")
        ...
        if config.gpu_id < 0 or config.gpu_id >= torch.cuda.device_count():
            raise ValueError(f"BasicVSR++ requested cuda:{config.gpu_id}, but {torch.cuda.device_count()} GPU(s) are visible")
        ...
        checkpoint = Path(config.model_path).expanduser().resolve() if config.model_path else download_checkpoint(checkpoint_dir)
        if not checkpoint.is_file():
            raise FileNotFoundError(f"BasicVSR++ checkpoint not found: {checkpoint}")
    ...
    def _enhance_with_tile(self, clip: torch.Tensor, tile_size: int) -> torch.Tensor:
    ...
    def _enhance_tensor(self, clip: torch.Tensor) -> torch.Tensor:
        ...
        for tile_size in candidates:
            try:
                result = self._enhance_with_tile(clip, tile_size)
    ...
    def enhance_clip(self, frames: Sequence[np.ndarray]) -> list[np.ndarray]:
        ...
        return [float_rgb_to_dtype(frame, first_dtype) for frame in mixed]
...
class BasicVSRPPStreamReader:
    """Convert a source frame reader into an overlapping, scene-aware restored stream."""
    ...
    def _regular_chunk(self) -> None:
        ...
        if self.first_chunk:
            self.output.extend(enhanced[: self.clip_length - self.overlap])
    ...
    def _finish_segment(self) -> None:
        if self.buffer:
            enhanced = self.preprocessor.enhance_clip(self.buffer)
            self.output.extend(enhanced if self.first_chunk else enhanced[self.overlap:])
        self.buffer = []
        self.first_chunk = True
        self.segment_end = False
