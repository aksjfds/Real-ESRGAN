...
主体线条使用 `realesr-animevideov3`，平面与色度使用 ArtCNN R16F96，通过
降噪 Kirsch 主轮廓软遮罩融合。双 GPU 模式会将连续帧分成两个独立任务，每张
T4 常驻 Real-ESRGAN 和 ArtCNN 模型。最终同样输出 8-bit HEVC。该流程质量
更可调，但显著慢于快速版。
...
本仓库保留 Real-ESRGAN 的 BSD 3-Clause `LICENSE`。ArtCNN 与 vs-mlrt 模型
分别从其官方发布地址按需下载，请同时遵守相应项目许可。
