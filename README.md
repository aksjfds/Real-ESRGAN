...
未实现的 Waifu2x、SwinIR、SCUNet、Restormer 和 Real-CUGAN 接口已从 CLI 删除。
tile 模式先在模型原生倍率完整拼接 FP32 帧，再执行一次全帧 Lanczos4。

### 快速版新增参数

| 分组 | 参数 |
| --- | --- |
| 预设 | `--quality-preset baseline\|safe` |
...
| 后处理 | `--anime4k`、`--anime4k-shader-dir`、`--anime4k-strength`、`--anime4k-shaders`、`--dehalo-strength`、`--dehalo-radius`、`--range-limit`、`--range-radius`、`--overshoot`、`--undershoot` |
...
快速版 Notebook 在 `RUN_BOTH_10S_TESTS=True` 且测试时长为 10 秒时，会依次生成
`realesrgan_baseline.mp4` 和 `realesrgan_safe.mp4`；执行单元不再显式覆盖预设控制的参数。
